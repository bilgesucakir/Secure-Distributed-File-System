...
namespace Server1_2
{
    public partial class Form1 : Form
    {
        ...
        private void getRSAKeys() //from text files
        {
            ...
            string workingDirectory = Environment.CurrentDirectory;              //reading "Server2_pub_prv.txt"
            string projectDirReal = Directory.GetParent(workingDirectory).Parent.FullName;
            string path = Path.Combine(projectDirReal, @"Server2_pub_prv.txt");
            if (File.Exists(path))
            {
                ...
                Server2PubPrivKey_XML = str1;
                Byte[] Server2PubPrivKey_XMLbyte = Encoding.Default.GetBytes(Server2PubPrivKey_XML);
                logs.AppendText("Server2's public-private key pair: " + generateHexStringFromByteArray(Server2PubPrivKey_XMLbyte) + "\n");
                ...
                if (File.Exists(path2))
                {
                    ...
                    string path3 = Path.Combine(projectDirReal3, @"Server1_pub.txt");
                    if (File.Exists(path3))
                    {
                        ...
                        Server1PubKey = str3;
                        Byte[] Server1PubKeybyte = Encoding.Default.GetBytes(Server1PubKey);
                        logs.AppendText("Server1's public key: " + generateHexStringFromByteArray(Server1PubKeybyte) + "\n");
                    }
            ...
            if (Int32.TryParse(masterPortNum.Text, out port))
            {
                try
                {
                    ...
                    while (!keyExchangeDone)
                    {
                        ...
                        buffer = Encoding.Default.GetBytes("Session Key Request, 2");   //Send session key request and ID of server
                        ...
                        byte[] decryptedSessionMessage = decryptWithRSA(sessionKeyMessage, 3072, Server2PubPrivKey_XML);
                        ...
                        if (verificationResult == true)   //if signiture verified, get session key(16 byte), IV(16 byte), HMAC(16 byte) from decrytedSessionMessage
                        {
                            ...
                            success = Encoding.Default.GetBytes("OK, 2");
                        ...
                        else
                        {
                            ...
                            Byte[] success = new Byte[48];
                            success = Encoding.Default.GetBytes("NOTOK, 2");
                            masterSocket.Send(success);
                        }
                    }
        ...
        private void Accept()
        {
            while (listening)
            {
                try
                {
                    ...
                    byte[] serverInfo = new Byte[64];
                    serverInfo = Encoding.Default.GetBytes("ServerTwo");
        ...
        private void Receive(Socket s)
        {
            ...
            while (connected && !terminating)
            {
                try
                {
                    ...
                    byte[] decFileAndNSize = decryptWithRSA(fileNameSize, 3072, Server2PubPrivKey_XML);
                    ...
                    byte[] decryptedAESKey = decryptWithRSA(AESKeyString, 3072, Server2PubPrivKey_XML);
                    ...
                    byte[] decryptedAESIV = decryptWithRSA(AESIVString, 3072, Server2PubPrivKey_XML);
        ...
        public byte[] generateNegativeAck(string failuremessage)
        {
            ...
            byte[] signednegativeack = signWithRSA(failuremessage, 3072, Server2PubPrivKey_XML);
        ...
        public byte[] generatePositiveAct(string positiveMessage)
        {
            ...
            byte[] signedPositiveAck = signWithRSA(positiveMessage, 3072, Server2PubPrivKey_XML);
